...
func TestDatabaseIntegration(t *testing.T) {
	...
	t.Run("UserRegistration", func(t *testing.T) {
		...
		assert.NoError(err).
			...
			Equal(user.Email, "alice@example.com").
			True(user.Active).
			True(user.ID > 0).
			True(time.Since(user.Created) < time.Minute)
	})
	...
	t.Run("DuplicateUserValidation", func(t *testing.T) {
		...
		assert.HasError(err).
			...
			ErrorContains(err, "bob")
	})
	
	t.Run("InputValidation", func(t *testing.T) {
		// Test empty username
		_, err := service.RegisterUser("", "test@example.com")
		assert.HasError(err).ErrorContains(err, "username is required")
		
		// Test empty email
		_, err = service.RegisterUser("testuser", "")
		assert.HasError(err).ErrorContains(err, "email is required")
		
		// Test invalid email format
		_, err = service.RegisterUser("testuser", "invalid-email")
		assert.HasError(err).ErrorContains(err, "invalid email format")
	})
	
	t.Run("ActiveUsersFiltering", func(t *testing.T) {
		// Create multiple users
		user1, _ := service.RegisterUser("active1", "active1@example.com")
		user2, _ := service.RegisterUser("active2", "active2@example.com")
		...
		assert.NoError(err).Len(activeUsers, 4) // alice, bob, active1, active2
...
func TestHTTPIntegration(t *testing.T) {
	...
	t.Run("SuccessfulUserCreation", func(t *testing.T) {
		...
		assert.NoError(err).
			Equal(resp.StatusCode, http.StatusCreated).
			Equal(resp.Header.Get("Content-Type"), "application/json")
		...
		assert.NoError(err).
			Equal(user.Username, "httpuser").
			Equal(user.Email, "httpuser@example.com").
			True(user.Active)
	})
	...
	t.Run("InvalidJSONRequest", func(t *testing.T) {
		...
		assert.NoError(err).
			Equal(resp.StatusCode, http.StatusBadRequest)
		resp.Body.Close()
	})
	...
	t.Run("MethodNotAllowed", func(t *testing.T) {
		...
		assert.NoError(err).
			Equal(resp.StatusCode, http.StatusMethodNotAllowed)
		resp.Body.Close()
	})
}
...
func TestConcurrentAccess(t *testing.T) {
	...
	t.Run("ConcurrentUserCreation", func(t *testing.T) {
		...
		assert.Len(errors, 0).Len(users, numGoroutines)
...
func TestAsyncOperations(t *testing.T) {
	...
	t.Run("EventualConsistency", func(t *testing.T) {
		...
		assert.NoError(err).
			Equal(createdUser.Username, "asyncuser").
			True(createdUser.ID > 0)
	})
